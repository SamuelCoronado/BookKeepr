...
const FormField = ({fieldLabel, fieldPlaceHolder, fieldType, selectOptions, validator, errorMessage, isFieldClean, handleField, index, onChange }) => {
        ...
        <div className={`field ${!isClean && 'error'}`} >
            {
                ...
                :
                <>
                <input type={fieldType === 'password'? type : fieldType} placeholder={fieldPlaceHolder} onChange={(e) => {handleField(index, validator(e.target.value)); setIsClean(isFieldClean[index]); onChange(e.target.value)}} className={!isClean && 'input-error'}/>
                {
